...
import datetime
import sys
sys.path.append( r'D:/Python/nlp/LLM' )
from zhipuUtils import zhipuLLM
...
# Sentence Generator (Decoder) for gemini-1.5-flash ...
def decoder_for_zhipu(args, input, max_length, i, k):
    
    for _ in range(5):
        try:
            # time.sleep(1)
            time.sleep(args.api_time_interval)
            response = zhipuLLM(input, max_tokens = max_length)
            return response
        except:
            time.sleep(2.0)
    return ""
...
# Sentence Generator (Decoder) for gemini-1.5-flash ...
def decoder_for_gemini(args, input, max_length, i, k):
...
class Decoder():
    ...
    def decode(self, args, input, max_length, i, k):
        ...
        response = decoder_for_zhipu(args, input, max_length, i, k)
        return response
...
def charmReader( args ):
    questions = []
    answers = []
    with open(args.dataset_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
        examples = data['examples']
        print(args.dataset_path, len(examples))
...
class MyDataset(Dataset):
    def __init__(self, args):
        super().__init__()
        if args.dataset.startswith( 'Chinese' ) or args.dataset.startswith( 'Global' ):
            self.questions, self.answers = charmReader( args )
        else:
...
def setup_data_loader(args):
    ...
    '''dataloader = torch.utils.data.DataLoader(dataset,
                  ...
                  pin_memory=True)'''
    dataloader = dataset
    dataloader = tuple( zip(dataloader[0], dataloader[1]) )
...
def answer_cleansing(args, pred):
    ...
    elif args.dataset in ("aqua", "commonsensqa"):
    ...
    if len(pred) == 0:
        ...
        if args.method in ("few_shot", "few_shot_cot"):
            ...
            if args.dataset in ("aqua", "commonsensqa"):
                ...
                pred = pred[-1]
                pred = pred.replace( "Therefore, the answer is", "")
                pred = pred.replace( '"', '' )
                pred = pred.replace( ' ', '' )
                pred = pred.strip()
                pred = pred.lower()
